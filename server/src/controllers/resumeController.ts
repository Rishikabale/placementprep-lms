...
export const analyzeResume = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        techKeywords.forEach(kw => {
            if (textLower.includes(kw)) {
                ...
                if (idx !== -1) extractedSkills.push(kw);
            }
        });
        ...
        if (hasEmail && hasPhone) structureScore += 5;
        ...
        if (hasProjects) {
            if (extractedSkills.length >= 3) projectsScore += 5; // Used a tech stack
            if (totalActionVerbs >= 3) projectsScore += 5; // Used action verbs inside projects
            if (isDenseDescription) projectsScore += 5; // Detailed descriptions (>30 words relative length)
            if (hasImpactOutcome) projectsScore += 5; // outcome/impact mentioned
        }

        // Experience Score (Max 15)
        let experienceScore = hasExperience ? 5 : 0;
        if (hasExperience) {
            if (totalActionVerbs >= 5) experienceScore += 5; // Role clarity via verbs
            if (hasImpactOutcome) experienceScore += 5; // Impact mentioned
        }
        ...
        let atsScore = 0;
        if (totalActionVerbs >= 8) atsScore += 5; // Action Verb Density
        if (resumeText.length > 1200 && resumeText.length < 5000) atsScore += 5; // Formatting density (not excessive)

        // Penalty Logic
        let penalty = 0;
        if (!hasEmail || !hasPhone) penalty += 15;
        if (!hasProjects) penalty += 15;
        if (!hasExperience) penalty += 10;
        if (resumeText.length < 800) penalty += 20; // Very short descriptions
        if (totalActionVerbs === 0) penalty += 15; // No action verbs
        if (extractedSkills.length < 3) penalty += 10; // Missing tech stack details
        ...
        if (totalScore >= 85) strengths.push('Outstanding Resume - Highly optimized for ATS and easily readable by human recruiters.');
        else if (totalScore >= 70) strengths.push('Good Resume - Strong foundation, hits most core requirements effectively.');
        else strengths.push('Functional Structure - Foundational elements exist but requires deep optimization.');

        if (totalActionVerbs >= 8) strengths.push('Excellent density of action-driven strong verbs (developed, designed, optimized, etc).');
        if (extractedSkills.length >= 8) strengths.push('High concentration of hard technical skills mapped correctly.');
        if (hasImpactOutcome) strengths.push('Clearly demonstrates Impact & Outcomes using quantifiable data/results.');

        // Meaningful / Non-Contradicting Flags
        if (!hasEmail || !hasPhone) weaknesses.push('Missing Contact Information: Cannot find standard email or phone number formats. You will be auto-discarded.');
        if (!hasLinkedIn && !hasGithub) weaknesses.push('Missing Social Proof: No GitHub or LinkedIn URLs detected.');
        if (resumeText.length < 800) weaknesses.push('Extremely Sparse Content: Resume lacks enough descriptive volume to accurately parse qualifications.');

        if (!hasProjects) weaknesses.push('Critically Missing Projects: Evaluators cannot see practical application of skills.');
        else if (projectsScore < 15) weaknesses.push('Weak Project Descriptions: Merely listing titles. Missing deep technical stack explanations and impacts.');

        if (!hasExperience) weaknesses.push('No formal Experience/Internship block identified. Highly penalized by ATS algorithms.');
        else if (experienceScore < 10) weaknesses.push('Experience lacks role clarity or quantified outcome/impact metrics.');

        if (totalActionVerbs < 3) weaknesses.push('Severe lack of Strong Action Verbs. Descriptions read as passive lists rather than achievements.');

        // Actionable Alignment (Step-by-Step guides for exactly what they should edit)
        if (!hasEmail || !hasPhone) suggestions.push('Immediately add your active Email Address and Phone Number at the very top of the document.');
        if (!hasLinkedIn && !hasGithub) suggestions.push('Include a link to your LinkedIn profile or GitHub portfolio to pass recruiter screening metrics.');
        if (!hasProjects) suggestions.push('Immediately build and document 1-2 core technical projects depending heavily on your target role.');
        if (projectsScore < 15 && hasProjects) suggestions.push('Refactor your Project bullet points. Mention EXACTLY what technologies you used, what you built, and what the final outcome was.');
        if (!hasImpactOutcome) suggestions.push('Modify your bullet points to show quantifiable results. Add numbers! (e.g., "Increased performance by 20%", "Reduced latency by 150ms").');
        if (totalActionVerbs < 5) suggestions.push('Review every bullet point in Experience and Projects. Ensure they start with a Strong Action Verb (e.g., Implemented, Spearheaded, Built, Optimized).');
        if (resumeText.length < 800) suggestions.push('Your resume is far too short (<800 chars). Expand your descriptions, list more collegiate coursework, or detail more technical specifications.');
        ...
        let feMatch = 0; let beMatch = 0; let dataMatch = 0;
        extractedSkills.forEach(s => {
            if (frontendWeights.includes(s)) feMatch += 1;
            if (backendWeights.includes(s)) beMatch += 1;
            if (dataWeights.includes(s)) dataMatch += 1;
        });
...
export const getResumeResult = async (req: Request, res: Response): Promise<any> => {
    try {
        const studentId = (req as any).user._id;
        const analysis = await ResumeAnalysis.findOne({ _id: req.params.id, studentId });
        if (!analysis) return res.status(404).json({ message: 'Requested analysis not found.' });
        res.json(analysis);
    } catch (error) {
...
export const getResumeHistory = async (req: Request, res: Response): Promise<any> => {
    try {
        const studentId = (req as any).user._id;
        // Don't pull huge raw text for history list endpoint to save bandwidth
        const history = await ResumeAnalysis.find({ studentId }).sort('-createdAt').select('-resumeText');
        res.json(history);
    } catch (error) {
